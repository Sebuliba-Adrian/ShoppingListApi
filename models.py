...
class Item(db.Model, AddUpdateDelete):
    id = db.Column(db.Integer, primary_key=True)
    ...
    category = db.relationship('Category', backref=db.backref(
        'items', lazy='dynamic', order_by='Item.item'))
    printed_times = db.Column(db.Integer, nullable=False, server_default='0')
    printed_once = db.Column(
        db.Boolean, nullable=False, server_default='false')

    def __init__(self, item, duration, category):
        self.item = item
        self.duration = duration
        self.category = category

    @classmethod
    def is_unique(cls, id, item):
        existing_item = cls.query.filter_by(item=item).first()
...
class ItemSchema(ma.Schema):
    id = fields.Integer(dump_only=True)
    item = fields.String(required=True, validate=validate.Length(1))
    ...
    url = ma.URLFor('api.itemresource', id='<id>', _external=True)
